...
SECTION_TO_TIMES = {
        ...
        7: "W/F 11-12:30P",
        8: "W/F 12:30-2P",
        9: "Tu 3:30-5P/Th 2-3:30P",
        10: "Tu 3:30-5P/Th 5-6:30P"
}
SECTION_TO_SECTION_NUMS = {
    0: [11, 12, 34, 39],
    1: [13, 14, 15, 35],
    2: [16, 17, 18, 36],
    3: [19, 20, 21, 37],
...
def key_from_value(search, list=SECTION_TO_TIMES):
...
def assign_placements(output_file):
    num_tas = len(all_tas)
    ...
    output_results(vars, output_file)


def output_results(vars, output_file):
    """
    ...
    with open(output_file, 'w') as f:
        for ta in all_tas:
            ...
            section_nums = ', '.join(str(SECTION_TO_SECTION_NUMS[id].pop(0)) for id in ta.placements)
            f.write('{0}, {1}, {2}\n'.format(ta.name, assigned, section_nums))
    print '=== Assignments finished ===	'
    for key, value in SECTION_TO_SECTION_NUMS.iteritems():
        if len(value) != 0:
            print 'warning: {0} has unused sections'.format(key)
...
if __name__ == '__main__':
    ...
    parser.add_argument('output_file', help='desired name of output file')
    ...
    assign_placements(args.output_file)
